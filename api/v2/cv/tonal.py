...
@bp.route("/skin_tone_opencv", methods=["POST"])
@doc(
...
def opencv_skin_tone_v2(image: FileStorage):
    log.debug("Start processing opencv_skin_tone_v2 request")
...
@bp.route("/skin_tone_mediapipe", methods=["POST"])
@doc(
    description="Search face on the photo and determine its skin color",
    tags=["tonal"],
    consumes=["multipart/form-data"],
...
@marshal_with(schema=ErrorSchema, code=422, description="Problem with image file")
@marshal_with(schema=ErrorSchema, code=400, description="No face on the image")
def mediapipe_skin_tone_v2(image: FileStorage):
    log.debug("Start processing mediapipe_skin_tone_v2 request")
    np_img = np.fromstring(image.read(), np.uint8)
    image = cv2.imdecode(np_img, cv2.IMREAD_COLOR)
    color = CVToneProcessor.mediapipe_tone_process(image)
    log.debug("Image was read from request body")
